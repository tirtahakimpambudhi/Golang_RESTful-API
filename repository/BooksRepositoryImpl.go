...
func (repository *BooksRepositoryImpl) FindAll(ctx context.Context, tx *sql.Tx , limit , offset int) []domain.Books {
...
func (repository *BooksRepositoryImpl) Count(ctx context.Context, tx *sql.Tx) int {
	var count int
	query := fmt.Sprintf("SELECT COUNT(*) FROM %v",config.TBNAME)
